...
GeoJSON under `public/data/` is generated by Node scripts in `scrapers/`. Run everything from the **repository root** after installing app dependencies (the IMD pipeline uses **SheetJS** `xlsx`, installed from the [official CDN tarball](https://cdn.sheetjs.com/) — not the stale `xlsx` copy on the public npm registry).
