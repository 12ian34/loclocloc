...
/**
 * Scraper for GP surgeries in London.
 * Uses the OpenStreetMap Overpass API.
 * Queries both amenity=doctors and healthcare=doctor tags.
 */
...
const OUTPUT_PATH = resolve(__dirname, "../public/data/gp-surgeries.geojson");
const LONDON_BBOX = "51.35,-0.42,51.65,0.25";

const QUERY = `
[out:json][timeout:120];
(
  node["amenity"="doctors"](${LONDON_BBOX});
  node["healthcare"="doctor"](${LONDON_BBOX});
);
out;
`;

async function main() {
  console.log("Scraping GP surgeries in London...\n");
  ...
  console.log(`Saved ${features.length} GP surgeries to ${OUTPUT_PATH}`);
}
